...
/* Q9: This time, produce the same result as in Q8, but using a subquery. */
    ...
    FROM 
        ...
        FROM Members AS m
        INNER JOIN Bookings AS b ON m.memid = b.memid
        INNER JOIN Facilities AS f ON b.facid = f.facid) AS outcome
    WHERE starttime LIKE '%2012-09-14%'
    AND cost >30
    ORDER BY cost DESC
...
that there's a different cost for guests and members! */
    ...
    FROM 
        ...
        FROM Bookings AS b
        INNER JOIN Facilities AS f ON b.facid = f.facid
        GROUP BY name) AS outcome
    WHERE revenue <1000
    ORDER BY revenue

/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */
    SELECT CONCAT( m.firstname, ' ', m.surname ) AS Recommended_By, CONCAT( recc.firstname, ' ', recc.surname) AS Member
    FROM Members m
    INNER JOIN Members recc ON recc.recommendedby = m.memid
    WHERE m.memid >0
    ORDER BY m.surname, m.firstname, recc.surname, recc.surname
    LIMIT 0 , 30
